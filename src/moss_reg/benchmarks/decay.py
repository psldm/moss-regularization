...
def run_decay_benchmark(
    ...
    ax = axes[1]
    ax.loglog(dts, errors, "o-", lw=2, label=f"Euler error (slope {slope:.2f})")
    ax.loglog(dts, errors[0] * (dts / dts[0]) ** 1.0, "k--", lw=1, label=r"$\propto \Delta t$")
    ...
    plt.close(fig)
    return {"speed0": speed0, "lambda": lam, "euler_slope": float(slope)}
