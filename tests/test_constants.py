...
def test_lambda_definition():
    assert K.LAMBDA == pytest.approx(K.G / K.C**3, rel=1e-15)
...
def test_lambda_times_planck_force_is_unity():
    ...
    assert 1.0 / K.F_P == pytest.approx(K.LAMBDA, rel=1e-15)


def test_lambda_c5_relation():
    # spec identity 1 / (8 pi Xi c) == G / c^5 == lambda * c^2
    assert K.lambda_c5 == pytest.approx(1.0 / (8.0 * math.pi * K.XI * K.C), rel=1e-15)
    assert K.lambda_c5 == pytest.approx(K.G / K.C**5, rel=1e-15)
    assert K.lambda_c5 == pytest.approx(K.LAMBDA * K.C**2, rel=1e-15)


def test_planck_length_time_mass():
    assert K.L_P == pytest.approx(math.sqrt(K.HBAR * K.G / K.C**3), rel=1e-15)
    assert K.T_P == pytest.approx(K.L_P / K.C, rel=1e-15)
    assert K.M_P == pytest.approx(math.sqrt(K.HBAR * K.C / K.G), rel=1e-15)
...
def test_planck_scale_reference_values():
    ...
    assert K.L_P == pytest.approx(1.616255e-35, rel=1e-4)
    assert K.T_P == pytest.approx(5.391247e-44, rel=1e-4)
    assert K.M_P == pytest.approx(2.176434e-8, rel=1e-4)
...
def test_dimensions_of_hbar():
    # [hbar] = J s = kg m^2 / s
    assert K.HBAR == pytest.approx(K.M_P * K.C * K.L_P, rel=1e-12)
...
def test_si_unit_system_matches_module_constants():
    ...
    assert si.lambda_ == pytest.approx(K.LAMBDA, rel=1e-15)
    assert si.l_p == pytest.approx(K.L_P, rel=1e-15)
